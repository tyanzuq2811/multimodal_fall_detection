...
#### Vấn đề 5: IMU có thang đo rất lệch giữa accel và gyro

Nhánh IMU ban đầu được đưa thẳng vào mạng 1D-CNN mà chưa chuẩn hóa đúng cách.

Hệ quả:

- Gia tốc kế thường nằm quanh vài đơn vị, trong khi gyro có thể lên tới hàng trăm đơn vị.
- Các kênh gyro áp đảo các kênh accel, làm mạng học lệch.
- Khi normalize theo batch, val set còn bị lệch nặng hơn vì batch có thể toàn mẫu ADL hoặc toàn mẫu fall.

#### Vấn đề 6: Fusion phụ thuộc trực tiếp vào chất lượng IMU

Nhánh Fusion lấy logit từ Pose và IMU để kết hợp.

Hệ quả:

- Nếu IMU yếu, Fusion sẽ bị kéo xuống và chỉ còn như camera-only.
- Nếu checkpoint IMU không được retrain đúng, phần MLP fusion học trên tín hiệu nhiễu.
...
#### Giải pháp F: Chuẩn hóa IMU theo từng cửa sổ và tăng độ dài huấn luyện

Với IMU, mình chuyển sang chuẩn hóa theo từng sample/cửa sổ (instance normalization trên trục thời gian) thay vì trộn cả batch.

Ý nghĩa:

- Triệt tiêu offset DC và phần gravity lệch theo tư thế đeo.
- Giảm hiện tượng batch thống kê bị lệch giữa train và val/test.
- Sau đó retrain IMU lâu hơn để mô hình có đủ thời gian hội tụ.

#### Giải pháp G: Retrain Fusion trên checkpoint IMU mới

Sau khi IMU đã được làm sạch và retrain, mình chạy lại Fusion để cập nhật trọng số cho tín hiệu IMU mới.

Tác dụng:

- Fusion không còn học trên nhánh IMU lỗi thời.
- Tăng độ ổn định của MLP fusion và giảm nguy cơ camera-only hóa hệ thống.
...
#### IMU retrain (validation subjects 9-10)
...
#### Fusion retrain (validation subjects 9-10)
...
Nhánh IMU và Fusion cũng đã được retrain theo đúng pipeline mới:

- IMU dùng instance normalization theo từng cửa sổ và đã cải thiện rõ rệt sau khi train dài hơn.
- Fusion được chạy lại trên checkpoint IMU mới để tránh học trên tín hiệu nhiễu.

Kết quả cuối cùng ở mức test mù subjects 11-12:
