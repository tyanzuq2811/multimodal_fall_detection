...
## Trạng Thái Hiện Tại
- **Pose**: đã retrain với kênh confidence, checkpoint tốt nhất ở epoch 130.
...
### Cấu hình mô hình
- **Checkpoint**: `weights/pose_finetuned_upfall_v2.pth`
...
- **Số epoch fine-tune**: 150, checkpoint tốt nhất ở epoch 130
...
|--------|------------|------|--------|---------|
| **Pose** | 0.9079 | 0.9453 | 0.96 | Mô hình triển khai chính |
...
## 5. Báo Cáo Kỹ Thuật (Tóm tắt v2)

### 5.1. Công nghệ sử dụng
- **Ngôn ngữ/Framework**: Python 3.x, PyTorch 2.6.0+cu124.
- **Pose**: Ultralytics YOLOv8 pose, OpenCV để đọc ảnh/video.
- **Tiền xử lý**: NumPy, scikit-learn cho metrics.
- **Dữ liệu**: Hugging Face datasets cho OmniFall.
- **Theo dõi huấn luyện**: CSV/JSON logs; W&B tùy chọn (nếu cài trong env).

### 5.2. Dữ liệu và kích thước
- **UP-Fall (raw)**: `data/raw/upfall` ~ 140G.
- **OmniFall (raw)**: `data/raw/omnifall` ~ 9.7G.
- **Pose v2 (processed)**: `data/processed/pose_features_v2` ~ 110M.
- **IMU (processed)**: `data/processed/imu_windows` ~ 5.0M.
- **Window manifest**: `data/processed/pose_features_v2/upfall_windows_v2.jsonl` ~ 15,408 cửa sổ.
- **Cấu trúc UP-Fall**: `SubjectX/ActivityY/TrialZ` chứa CSV + zip camera 1/2.
- **Kết quả trích xuất v2**: 574/576 file pose (2 zip lỗi bị skip).
- **OmniFall v2**: 4,500 file pose, manifest 4,493 mẫu (7 video lỗi moov atom).

### 5.3. Pipeline trích xuất v2 (chuẩn hóa YOLO)
- Chạy YOLO pose với `conf=0.5`, `classes=[0]`, không lọc keypoint theo conf.
- Lưu **toàn bộ (x, y, conf)** cho 17 khớp.
- Nếu không phát hiện người: chèn `np.zeros((17, 3))` để giữ nhịp thời gian.
- Chuẩn hóa tọa độ về [0, 1]; resample thành 60 frames / window.

### 5.4. Vì sao chọn conf = 0.5 (không phải 0.7)
- **0.5 giữ Recall**: 0.7 bỏ nhiều frame có người nhưng che khuất, làm mất tín hiệu.
- **Khai thác kênh conf**: mục tiêu là cho mạng tự học mức tin cậy, không cắt bỏ.
- **Ổn định temporal**: ngưỡng cao gây nhiều frame rỗng, tăng số zero-fill.
- **Chuẩn YOLO phổ biến**: 0.5 là điểm cân bằng noise/recall trong thực tế.

### 5.5. Vấn đề dữ liệu và cách xử lý
- **BadZipFile (UP-Fall)**: 2 zip camera lỗi -> skip, ghi log.
- **moov atom not found (OmniFall)**: video lỗi metadata -> bỏ qua mẫu.
- **HF rate limit**: khuyến nghị dùng `HF_TOKEN` khi tải dataset.

### 5.6. Kết quả v2 (tham chiếu nội bộ)
- **Pretrain OmniFall**: best epoch 35, acc 0.9933, F1 0.9907, val loss 0.0135.
