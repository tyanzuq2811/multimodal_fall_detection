...
class OmniFallPoseDataset(Dataset):
    ...
    def __getitem__(self, idx: int):
        item = self.items[idx]
        t, kpts = _load_pose_npz(item.pose_path)
        if t.size == 0 or t[-1] <= t[0]:
            # Empty or invalid time range -> return zero pose
            pose = np.zeros((self.pose_target_len, kpts.shape[1], 2), dtype=np.float32)
        else:
            pose = resample_pose(t, kpts, float(t[0]), float(t[-1]), self.pose_target_len, mode="nearest")
        pose = np.transpose(pose, (2, 0, 1))  # (2, T, J)
        return {
